...
export default function TotalGrowthScreen({ route, navigation }) {
  ...
  const Action_continue = () => {
    ...
    navigation.navigate('RecentAllGrowthScreen', { child_id: state.child_id, onGoBack: undefined })
  }
  const Action_Addcontinue = () => {
    action_event('Add Growth Update')
    navigation.navigate('ListGrowthSelectionScreen', { child_id: state.child_id, type: 'add', gender: state.gender, age: state.age })
  }
  const click_Weight = () => {
    action_event('Weight')
    state.istype = ''
    ...
    <View style={stylesBackground.container}>
      ...
      <View style={{ width: deviceWidth - 34, alignItems: 'center', alignSelf: 'center' }}>
        <View style={{ width: '100%', alignItems: 'flex-start' }}>
          <Text style={styles.mainHeadertext} adjustsFontSizeToFit={true} numberOfLines={1}>{'Growth Chart'}</Text>
        </View>
      </View>
      <View style={styles.container}>
        <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 30 }}>
          ...
          <Text style={{ color: colors.Blue, fontFamily: fonts.rubikSemiBold, fontSize: 20, marginStart: 10 }}>{state.name}</Text>
        </View>
        <View style={{ width: deviceWidth - 34, height: 50, marginTop: 30, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
          <TouchableWithoutFeedback onPress={() => click_Weight()}>
            <View style={state.istype == '' ? styles.activebtnStyle : styles.inactivebtnStyle}>
              <Text style={state.istype == '' ? styles.activeStyle : styles.inactiveStyle}>{'Weight'}</Text>
            </View>
          </TouchableWithoutFeedback>
          <TouchableWithoutFeedback onPress={() => click_Height()}>

            <View style={state.istype == '1' ? styles.activebtnStyle : styles.inactivebtnStyle}>
              <Text style={state.istype == '1' ? styles.activeStyle : styles.inactiveStyle}>{'Height'}</Text>
            </View>
          </TouchableWithoutFeedback>
          <TouchableWithoutFeedback onPress={() => click_Head()}>

            <View style={state.istype == '2' ? styles.activebtnStyle : styles.inactivebtnStyle}>
              <Text style={state.istype == '2' ? styles.activeStyle : styles.inactiveStyle}>{'Head Size'}</Text>
            </View>
          </TouchableWithoutFeedback>
        ...
        {state.dataGraph.length > 0 ?
          <View style={{ flex: 1 }}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} bounces={true} style={{ flex: 1, }}>
              ...
              <View style={{ marginTop: 10, alignItems: 'center', height: deviceHeight / 2.3, marginBottom: 165, }} >
                <LineChart
                  curved
                  isAnimated
                  ...
                  xAxisLabelTextStyle={{ color: colors.Blue, fontSize: 9, fontFamily: fonts.rubikBold, textAlign: 'center' }}
                  ...
                  key={'xyz'}
                />
                <View style={{ width: deviceWidth - 34, height: 50, flexDirection: 'row', alignItems: 'center', marginStart: 20 }}>
                  <Text style={{ fontFamily: fonts.rubikSemiBold, fontSize: 16, color: colors.Blue }}>{'Percentile'}</Text>

                  <View style={{ flexDirection: 'row', marginStart: 21 }}>
                    <View style={{ height: 33.33, width: 33.33, borderRadius: 33.33 / 2, backgroundColor: colors.Darkpink, marginEnd: 4, alignItems: 'center', justifyContent: 'center' }}>
                      <Text style={{ fontFamily: fonts.rubikRegular, fontSize: 10, color: colors.Blue }}>{'5th'}</Text>
                    </View>
                  </View>
                  <View style={{ flexDirection: 'row', marginStart: 17 }}>
                    <View style={{ height: 33.33, width: 33.33, borderRadius: 33.33 / 2, backgroundColor: '#E9E9E9', marginEnd: 4, alignItems: 'center', justifyContent: 'center' }}>
                      <Text style={{ fontFamily: fonts.rubikRegular, fontSize: 10, color: colors.Blue }}>{'95th'}</Text>
                    </View>
                  </View>
                  <View style={{ flexDirection: 'row', marginStart: 17 }}>
                    <View style={{ height: 33.33, width: 33.33, borderRadius: 33.33 / 2, backgroundColor: colors.Blue, marginEnd: 4, alignItems: 'center', justifyContent: 'center' }}>
                      <Text style={{ fontFamily: fonts.rubikRegular, fontSize: 10, color: colors.White }}>{'Baby'}</Text>
                    </View>
                  </View>
                </View>
              </View>
        ...
        <BottomButton text={'Add Growth Update'} onPress={() => Action_Addcontinue()} container={{ position: 'absolute', bottom: 50, backgroundColor: colors.textinputBackground, borderWidth: 1, borderColor: colors.Blue }} textstyle={{ color: colors.Blue }} />

        <BottomButton text={'See recent growth entries'} onPress={() => Action_continue()} container={{ position: 'absolute', bottom: -20 }} />
...
const styles = StyleSheet.create({
  ...
  activebtnStyle: { backgroundColor: colors.Blue, height: 45, borderRadius: 10, justifyContent: 'center', alignItems: 'center', width: '30%' },
  inactivebtnStyle: { height: 45, borderRadius: 10, borderWidth: 1, borderColor: colors.Blue, justifyContent: 'center', alignItems: 'center', width: '30%' }
});
